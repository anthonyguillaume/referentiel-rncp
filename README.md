...
`docs/diplomes_rncp.xlsx` — le classeur complet. Onglet **Diplômes** : une ligne par fiche (code, intitulé, type, niveau, fiche active, échéance, remplacée par, formacodes, lien France compétences cliquable, évolution et son détail avant → après, date du dernier changement, suivi depuis), sous forme de tableau Excel filtrable. Code couleur **cumulé depuis la référence de départ** (l'export initial, ou le fichier Excel importé avec `outils/initialiser_depuis_excel.py`) : **vert** = fiche absente de la référence, **orange** = fiche modifiée depuis (détail avant → après cumulé), **rouge** = fiche disparue de l'export. Ces couleurs persistent jusqu'à la prochaine réinitialisation de la référence ; la colonne « Dernier changement » et la page Historique donnent ce qui a bougé un jour donné. L'onglet **Changements** est le journal daté complet (lignes de la génération courante surlignées en bleu), l'onglet **Synthèse** donne les compteurs (cumul et génération du jour) et la liste des lignes ayant évolué, et un onglet par domaine (« Formacode 110 », « Formacode 114 »…) reprend la vue par formacode simplifié.
...
`docs/index.html` + `docs/data.json` — le **site web** (servi par GitHub Pages) : recherche instantanée par code, intitulé ou formacode, filtres Ajoutés / Modifiés / Supprimés / Fiches inactives, Type et Niveau, détail par fiche avec lien vers France compétences, et bouton de téléchargement de l'Excel. Le bandeau rappelle la référence en cours ; si la dernière exécution a échoué, le point de la puce « Généré le » passe au rouge et une alerte s'affiche.
